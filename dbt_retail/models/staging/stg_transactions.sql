...
typed as (
  select
    ...
    -- scenario handling for amount_cents
    {% if sc == 'model_bug' %}
    {{ exceptions.raise_compiler_error('model_bug scenario: intentional failure due to missing column') }}
    {% elif sc == 'logic_bug' %}
    ...
    {% else %}
    -- normal or bad_data scenario: cast amount_cents safely
    cast(amount_cents as integer) as amount_cents,
    {% endif %}
