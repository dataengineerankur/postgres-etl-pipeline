...
typed as (
  select
    ...
    -- model_bug scenario: raise explicit compile error instead of referencing a non‑existent column
    {% if sc == 'model_bug' %}
    {{ exceptions.raise_compiler_error('model_bug scenario intentionally fails due to column mismatch') }}
    {% elif sc == 'logic_bug' %}
