...
typed as (
  select
    ...
    -- model_bug scenario: raise explicit compile-time error instead of referencing a missing column
    {% if sc == 'model_bug' %}
    {{ exceptions.raise_compilation_error('model_bug scenario triggered: intentional failure') }}
    {% elif sc == 'logic_bug' %}
