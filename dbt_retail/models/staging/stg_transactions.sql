{#-
  ...
  - scenario=model_bug: intentionally raises a compile-time error to simulate a regression.
...
typed as (
  select
    ...
    {% if sc == 'model_bug' %}
    {{ exceptions.raise_compiler_error('model_bug scenario: intentional compile-time failure') }}
    {% elif sc == 'logic_bug' %}
    ...
    {% else %}
    -- normal processing (also covers bad_data scenario where casting may fail)
    cast(amount_cents as integer) as amount_cents,
    {% endif %}
