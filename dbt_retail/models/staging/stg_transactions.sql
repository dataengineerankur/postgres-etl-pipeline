...
typed as (
  select
    ...
    -- model_bug scenario: now reference the correct column but raise an explicit error to preserve failure semantics
    {% if sc == 'model_bug' %}
    cast(amount_cents as integer) as amount_cents,
    {{ exceptions.raise_compiler_error('model_bug scenario triggered intentionally') }}
    {% elif sc == 'logic_bug' %}
