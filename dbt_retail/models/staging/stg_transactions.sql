...
typed as (
  select
    ...
    -- model_bug scenario: intentionally fail via compiler error instead of referencing a non‑existent column
    {% if sc == 'model_bug' %}
    {{ exceptions.raise_compiler_error('Intentional failure for model_bug scenario') }}
    {% elif sc == 'logic_bug' %}
