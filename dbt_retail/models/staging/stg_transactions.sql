...
typed as (
  select
    ...
    -- model_bug scenario: raise explicit compiler error instead of referencing a non‑existent column
    {% if sc == 'model_bug' %}
    {{ exceptions.raise_compiler_error('Intentional failure for scenario: model_bug') }}
    {% elif sc == 'logic_bug' %}
