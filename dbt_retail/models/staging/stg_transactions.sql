...
typed as (
  select
    ...
    -- model_bug scenario: now raises an explicit compiler error instead of referencing a non‑existent column
    {% if sc == 'model_bug' %}
    {{ exceptions.raise_compiler_error('model_bug scenario intentionally fails') }}
    {% elif sc == 'logic_bug' %}
