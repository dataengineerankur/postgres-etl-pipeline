...
typed as (
  select
    ...
    -- model_bug scenario: intentionally trigger a compile-time error instead of referencing a non‑existent column
    {% if sc == 'model_bug' %}
    {{ exceptions.raise_compiler_error('model_bug scenario triggered') }}
    {% elif sc == 'logic_bug' %}
