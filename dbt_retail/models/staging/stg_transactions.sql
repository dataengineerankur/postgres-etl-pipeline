...
typed as (
  select
    ...
    -- scenario handling
    {% if sc == 'model_bug' %}
    {{ exceptions.raise_compiler_error('model_bug scenario intentionally fails: incorrect column reference') }}
    {% elif sc == 'logic_bug' %}
    ...
    {% else %}
    -- normal path (including bad_data scenario where casting may fail)
    cast(amount_cents as integer) as amount_cents,
    {% endif %}
