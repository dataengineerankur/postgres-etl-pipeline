...
typed as (
  select
    ...
    {% if sc == 'model_bug' %}
    -- intentionally cause a runtime cast error to preserve failure semantics without referencing a missing column
    cast('invalid_amount' as integer) as amount_cents,
    {% elif sc == 'logic_bug' %}
